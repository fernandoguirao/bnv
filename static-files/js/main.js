...
setTimeout(function() {
	jQuery.each(arr, function(indexInArray) {
		setTimeout(function() {
			...
			var a = actualarr[1], b = actualarr[2];
...
/* $('.timelementos').pep({constrainToParent: true,axis: 'x'}); */
	$('.timelementos').pep({
		...
		stop: function(ev,obj){ 
			...
			/* obj.forceStop(); */
				$('.timelementos').css({"left":$(window).width()+60});
				/* $('.capacidadeslink').click(); */
			}
		},
...
$(window).scroll(function () {
	...
	if ($(this).scrollTop() > 1750 /* && $(this).scrollTop() < 2300 */ ) {
		$('.separaador').stop().animate({"height":"193px"},function(){
...
*/
		...
		$('#menuequipo').stop().animate({'bottom':'-111'},function(){
			$(this).css({
				"position":"absolute",
				"z-index":"-1"
			})
...
$(window).scroll(function () {
	if ($(this).scrollTop() > 1000 && $(this).scrollTop() < 1900 ) {
		...
		$('.timelementos').css({"left":+3641-((($(this).scrollTop())*1.2) - 1000)});
	} 
})
